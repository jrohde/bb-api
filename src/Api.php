...
class Api
{
     ...
     * Api URL
     *
     * @example https://www.bb.tld/api
     * @var string
     */
    protected $_api_url     = NULL;

    /**
     * Api Token found in BB profile page
     *
    ...
    /**
     * Same service can be used to control BB as client and guest
     *
     * @example guest
     * @example admin
     * @example client
    ...
    public function __call($method, $arguments)
    {
        ...
        if(!is_object($json)) {
            throw new Exception(sprintf('BB API: Invalid Response "%s"', $result));
        }

        if(isset($json->error) && !empty($json->error)) {
            throw new Exception(sprintf('BB API method "%s" returned error: "%s"', $method, $json->error->message), $json->error->code);
        }
